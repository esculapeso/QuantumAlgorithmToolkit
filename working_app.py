"""
Simple working application for quantum simulations.
This standalone file will run your simulations and display plots.
"""
...
def view_result(result_name):
    ...
    try:
        ...
        result_data = {
            ...
            'qubits': int(metadata.get('qubits', 0)),
            'drive_param': float(metadata.get('drive_param', 0)),
            'max_time': float(metadata.get('max_time', 0)),
            'time_points': int(metadata.get('time_points', 0)),
            'time_crystal': metadata.get('time_crystal_detected') == 'True',
            'incommensurate': int(metadata.get('incommensurate_count', 0)),
